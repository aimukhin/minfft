# Single precision split-radix (2/4) DIF forward FFT
# Arch: x86-64 SSE3
# ABI: SysV AMD64
#
...
L:	# loop N/4 times (N>=16) with 2 iterations in parallel
	...
	subps		%xmm3,%xmm2		# xmm2=t2-t3,'
	addps		%xmm3,%xmm4		# xmm4=t2+t3,'
	movaps		%xmm10,%xmm12
	subps		%xmm11,%xmm10
	addps		%xmm11,%xmm12
	...
	push		%r8
	push		%r9
	# call rs_dft_1d(N/2,x,y,2sy,e+N/2)
	call		rs_dft_1d
	pop		%r9			# r9=e+N/2
	pop		%r8			# r8=2sy
	pop		%rcx			# rcx=y
	...
	push		%r8
	push		%r9
	# call rs_dft_1d(N/4,x+N/2,y+sy,4sy,e+3N/4)
	call		rs_dft_1d
	pop		%r9			# r9=e+3N/4
	pop		%r8			# r8=4sy
	pop		%rcx			# rcx=y+sy
	...
	# call rs_dft_1d(N/4,x+3N/4,y+3sy,4sy,e+3N/4)
	jmp		rs_dft_1d
	# we're done
...
T4:	# N==4
	...
	movhps		%xmm2,(%rcx,%r8)	# y[sy]=t2-t3
	lea		(%rcx,%r8,2),%rcx	# advance pointer
	movlps		%xmm2,(%rcx)		# y[2*sy]=t0-t1
	movhps		%xmm0,(%rcx,%r8)	# y[3*sy]=t2+t3
	ret
...
T8:	# N==8
	...
	addps		%xmm1,%xmm0		# xmm0=t0+t1,t2+t3=t00,t03
	subps		%xmm1,%xmm2		# xmm2=t0-t1,t2-t3=t02,t01
	addps		%xmm5,%xmm4		# xmm4=t0'+t1',t2'+t3'=t10,t13
	subps		%xmm5,%xmm6		# xmm6=t0'-t1',t2'-t3'=t12,t11
	# rearrange for complex multiplications
	movapd		%xmm4,%xmm7		# xmm7=t10,t13
	movhlps		%xmm6,%xmm7		# xmm7=t11,t13
	...
	# multiply t12 by -I
	shufps		$0xE1,%xmm6,%xmm6	# swap Re and Im of lower complex
	xorps		li(%rip),%xmm6		# flip sign of Im of lower complex
	# rearrange
	movlhps		%xmm8,%xmm6		# xmm6=t12,t11*e1
	shufps		$0xE4,%xmm8,%xmm4	# xmm4=t10,t13*e3
	# add, subtract, and store
	movaps		%xmm0,%xmm1		# xmm1=t00,t03
	movaps		%xmm2,%xmm3		# xmm3=x02,t01
	addps		%xmm4,%xmm0		# xmm0=t00+t10,t03+t13
	addps		%xmm6,%xmm2		# xmm2=t02+t12,t01+t11
	subps		%xmm4,%xmm1		# xmm1=t00-t10,t03-t13
	subps		%xmm6,%xmm3		# xmm3=t02-t12,t01-t11
	movlps		%xmm0,(%rcx)		# store y[0]
	lea		(%rcx,%r8,8),%rcx	# advance pointer
	movhps		%xmm2,(%rcx)		# store y[sy]
	lea		(%rcx,%r8,8),%rcx	# advance pointer
	movlps		%xmm2,(%rcx)		# store y[2*sy]
	lea		(%rcx,%r8,8),%rcx	# advance pointer
	movhps		%xmm0,(%rcx)		# store y[3*sy]
	lea		(%rcx,%r8,8),%rcx	# advance pointer
	movlps		%xmm1,(%rcx)		# store y[4*sy]
	lea		(%rcx,%r8,8),%rcx	# advance pointer
	movhps		%xmm3,(%rcx)		# store y[5*sy]
	lea		(%rcx,%r8,8),%rcx	# advance pointer
	movlps		%xmm3,(%rcx)		# store y[6*sy]
	lea		(%rcx,%r8,8),%rcx	# advance pointer
	movhps		%xmm1,(%rcx)		# store y[7*sy]
	# done
	ret
...
hr:	.long		0x00000000
	...
	.long		0x80000000
	.long		0x00000000
# sign-flipping mask for low imaginary part
li:	.long		0x00000000
	.long		0x80000000
	.long		0x00000000
	.long		0x00000000
